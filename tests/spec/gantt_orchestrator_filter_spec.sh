Describe 'generate-dashboard.sh: Orchestrator Appears in Gantt Chart (T4)'
  setup() {
    ...
    # board.jsonl: data containing orchestrator checkpoint entries
    # Before the T4 fix, orchestrator was excluded by the agents filter,
    # and not displayed in Gantt chart Y-axis labels
...
BOARD
  ...
  It 'filters only human from agents, not orchestrator'
    ...
    # Verify that renderGantt() agents filter does not contain orchestrator exclusion
    # Before fix: return a !== 'human' && a !== 'orchestrator'
    # After fix: return a !== 'human'
