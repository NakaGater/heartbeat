Describe 'dashboard.html: Bentoグリッド + サイドバーのCSS実装（タスク3）'
  ...
  It '.bento-card.card-backlog に grid-column: span 7 が定義されている'
    extract_backlog_col_span() {
      awk '/\.bento-card\.card-backlog|\.card-backlog/{found=1} found && /grid-column[ \t]*:.*span 7/{print "ok"; exit} found && /\}/{found=0}' "$TEMPLATE"
    }
    When call extract_backlog_col_span
    The output should equal "ok"
  End

  It '.bento-card.card-backlog に grid-column: span 7 が定義されている (grid-row: span 2 は削除済み)'
    extract_backlog_col_only() {
      awk '/\.bento-card\.card-backlog|\.card-backlog/{found=1} found && /grid-column[ \t]*:.*span 7/{print "ok"; exit} found && /\}/{found=0}' "$TEMPLATE"
    }
    When call extract_backlog_col_only
    The output should equal "ok"
  End
  ...
  It '.bento-card.card-velocity に grid-column: span 5 が定義されている'
    extract_velocity_col_span() {
      awk '/\.bento-card\.card-velocity|\.card-velocity/{found=1} found && /grid-column[ \t]*:.*span 5/{print "ok"; exit} found && /\}/{found=0}' "$TEMPLATE"
    }
