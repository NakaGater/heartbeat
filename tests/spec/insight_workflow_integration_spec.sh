Describe 'Independent Workflow Integration Test (0045-pdm-insight-analysis Task 6)'
  ...
  # --- E2E test data injection helper ---
  # Inject 4 layers sequentially using analyze.sh, then generate summary via summary.sh
  run_full_pipeline() {
    # Raw layer: 2 entries simulating text file input
    ...
    # Findings layer: 3 entries with Raw references
    ...
    # Insights layer: 2 entries with Findings references
    ...
    # Opportunities layer: 1 entry with Insights references
  ...
  Describe 'Traceability: Cross-layer Reference Chain Consistency (OPP -> INS -> FND -> RAW)'
    It 'verifies that Opportunities source_insights references existing INS-NNN'
      run_and_verify_opp_refs() {
        run_full_pipeline
        # Check if each ID in OPP-001 source_insights exists in insights.jsonl
    ...
    It 'verifies that Insights source_findings references existing FND-NNN'
      run_and_verify_ins_refs() {
        run_full_pipeline
        # Piped while-read runs in subshell so return 1 does not propagate.
        # Use a for loop to work around this.
    ...
    It 'traces the complete evidence chain OPP -> INS -> FND -> RAW'
      run_and_verify_full_chain() {
        run_full_pipeline
        # Verify the chain OPP-001 -> INS-001 -> FND-001 -> RAW-001
  ...
  Describe 'Figma: --figma Option Graceful Degradation Without MCP'
    It 'does not exit non-zero when running analyze.sh raw with --figma option'
      # Since analyze.sh is designed to accept layer + json_entry,
      # --figma is handled by the skill layer. Verify the script accepts source_type=figjam entries
