Describe 'Velocity Chart Integration Test: Real Data Verification via generate-dashboard.sh (Task 5)'
  ...
  setup() {
    ...
    # backlog.jsonl simulating real data: mixed points strings, mixed TZ, null values
    # Week assignment (UTC-based):
    #   2026-W13: s1(3) + s2(2) = 5
    #   2026-W14: s3(1) + s5(4) = 5   (s4 excluded because points=null)
...
JSONL
  ...
  # AC-10: Velocity Chart renders correctly
  # AC-11: Velocity is correctly calculated with pre-fix data (equivalent to existing backlog.jsonl)
  #
  # Verification:
  # 1. dashboard.html is actually generated by generate-dashboard.sh
  # 2. Generated HTML contains SVG for Velocity Chart
  # 3. Value labels are correct numbers (not string concatenation)
  # 4. Average value is numerically correct (avg 5.0)
  # 5. Labels are in YYYY-Wnn format
  check_velocity_integration() {
    ...
    # Verify Velocity Chart section from generated HTML via node
    node -e "
      var fs = require('fs');
