...
## テスト: refactor.md にテスト実行フォアグラウンドルールが定義されていること
## ストーリー: 0049-subagent-test-foreground / タスク 3

# 条件1: refactor.md に「## Test Execution Rule」セクションが存在すること
...
# 条件3: 既存の「Refactoring Rules」セクションが維持されていること
check_refactor_has_refactoring_rules() {
  grep -q "^## Refactoring Rules$" "core/agent-personas/refactor.md" || return 1
}

Describe 'refactor.md テスト実行フォアグラウンドルール — セクション存在'
  It 'refactor.md に「## Test Execution Rule」セクションが存在する'
    When call check_refactor_has_test_execution_rule_section
    The status should be success
  End
End

Describe 'refactor.md テスト実行フォアグラウンドルール — 必須指示'
  It 'フォアグラウンド（同期）実行の指示が含まれる'
...
Describe 'refactor.md テスト実行フォアグラウンドルール — 既存セクション維持'
  It '既存の「Refactoring Rules」セクションが維持されている'
    When call check_refactor_has_refactoring_rules
    The status should be success
  End
End

## ネガティブテスト: reviewer.md / qa.md には「Test Execution Rule」セクションが存在しないこと
## ストーリー: 0049-subagent-test-foreground / タスク 4

# 条件5: reviewer.md に「## Test Execution Rule」セクションが存在しないこと
check_reviewer_does_not_have_test_execution_rule_section() {
  ! grep -q "^## Test Execution Rule$" "core/agent-personas/reviewer.md"
}

# 条件6: qa.md に「## Test Execution Rule」セクションが存在しないこと
check_qa_does_not_have_test_execution_rule_section() {
  ! grep -q "^## Test Execution Rule$" "core/agent-personas/qa.md"
}

Describe 'reviewer.md / qa.md — Test Execution Rule セクション不在（ネガティブテスト）'
  It 'reviewer.md に「## Test Execution Rule」セクションが存在しない'
    When call check_reviewer_does_not_have_test_execution_rule_section
    The status should be success
  End

  It 'qa.md に「## Test Execution Rule」セクションが存在しない'
