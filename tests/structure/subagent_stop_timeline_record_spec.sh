...
CLAUDE_SETTINGS_LOCAL="adapters/claude-code/hooks/settings.local.json"

# --- Helper functions ---

check_subagent_stop_has_timeline_record() {
  jq -e '.hooks.SubagentStop[].hooks[] | select(.command == "./core/scripts/timeline-record.sh")' \
    "$CLAUDE_SETTINGS" >/dev/null 2>&1
}

check_subagent_stop_has_timeline_record_local() {
  jq -e '.hooks.SubagentStop[].hooks[] | select(.command == "./core/scripts/timeline-record.sh")' \
    "$CLAUDE_SETTINGS_LOCAL" >/dev/null 2>&1
}

check_timeline_record_before_board_stamp() {
  timeline_idx=$(jq '[.hooks.SubagentStop[0].hooks[] | .command] | to_entries[] | select(.value == "./core/scripts/timeline-record.sh") | .key' "$CLAUDE_SETTINGS" 2>/dev/null)
...
check_timeline_record_is_first_in_subagent_stop() {
  ...
  [ "$first_cmd" = "./core/scripts/timeline-record.sh" ]
}

check_timeline_record_is_first_in_subagent_stop_local() {
  first_cmd=$(jq -r '.hooks.SubagentStop[0].hooks[0].command' "$CLAUDE_SETTINGS_LOCAL" 2>/dev/null)
  [ "$first_cmd" = "./core/scripts/timeline-record.sh" ]
}
...
check_settings_local_subagent_start_has_timeline_record() {
...
Describe 'SubagentStop hook wiring for timeline-record.sh'
  ...
  It 'settings.local.json の SubagentStop フックに timeline-record.sh が登録されていること'
    When call check_subagent_stop_has_timeline_record_local
    The status should be success
  End
  ...
  It 'settings.local.json で timeline-record.sh が SubagentStop フックの先頭（インデックス 0）であること'
    When call check_timeline_record_is_first_in_subagent_stop_local
    The status should be success
  End
  ...
  It 'settings.local.json の SubagentStart に timeline-record.sh が登録されていること'
    When call check_settings_local_subagent_start_has_timeline_record
    The status should be success
  End
End
