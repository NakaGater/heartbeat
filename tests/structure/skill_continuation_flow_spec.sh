...
# --- タスク4: Orchestrator responsibilities step 7 の更新 ---

extract_orchestrator_section() {
  # Orchestrator responsibilities セクション（### Orchestrator responsibilities から次の ## セクションまで）を抽出
...
check_orch_wf2_continuation_flow() {
  # Orchestrator responsibilities 内の Workflow 2 停止条件が Continuation Flow を参照していること
  extract_orchestrator_section || return 1
  # step 7 内に "Workflow 2" と "Continuation Flow" が同じコンテキストで言及されていること
  grep -i "workflow 2" /tmp/orch_section.tmp | grep -qi "continuation flow"
}

check_orch_wf3_continuation_flow() {
  # Orchestrator responsibilities 内の Workflow 3 停止条件が Continuation Flow を参照していること
  extract_orchestrator_section || return 1
  grep -i "workflow 3" /tmp/orch_section.tmp | grep -qi "continuation flow"
}

check_orch_wf1_stop_unchanged() {
  # Orchestrator responsibilities 内の Workflow 1 停止条件は STOP のままであること
  extract_orchestrator_section || return 1
  grep -i "workflow 1" /tmp/orch_section.tmp | grep -q "STOP"
}

Describe 'SKILL.md Orchestrator responsibilities step 7 の更新（タスク4）'
  It 'Workflow 2 の停止条件が Continuation Flow を参照している'
    When call check_orch_wf2_continuation_flow
    The status should be success
  End

  It 'Workflow 3 の停止条件が Continuation Flow を参照している'
    When call check_orch_wf3_continuation_flow
    The status should be success
  End

  It 'Workflow 1 の停止条件は STOP のまま変更されていない'
