...
check_workflow1_stop() {
  grep -A 50 "## Workflow 1" "$SKILL_MD" | grep -q "STOP"
}

check_workflow1_end() {
  grep -A 60 "## Workflow 1" "$SKILL_MD" | grep -q "END OF WORKFLOW 1"
}
...
check_agent_startup_workflow_branch() {
  grep -A 60 "## Agent Startup Method" "$SKILL_MD" | grep -q "Check current workflow context"
}
