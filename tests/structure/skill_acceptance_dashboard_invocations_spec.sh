...
check_total_dashboard_invocations_is_five() {
  # SKILL.md 全体で generate-dashboard.sh の呼び出しが正確に 5 箇所であること
  # (既存4箇所 + Orchestrator responsibilities 1箇所)
...
Describe '受け入れ基準: 全遷移ポイントの網羅確認'
  It 'generate-dashboard.sh の呼び出し総数が 5 である (既存4 + Orchestrator責務1)'
    When call check_total_dashboard_invocations_is_five
    The status should be success
  End
End
