...
check_total_dashboard_invocations_is_nine() {
  # SKILL.md 全体で generate-dashboard.sh の呼び出しが正確に 9 箇所であること
  # (既存4 + Orchestrator責務1 + Workflow 3 in_progress遷移1 + Workflow 6 skip説明1 + Workflow 6 post-merge1 + Continuation Flow1)
...
Describe '受け入れ基準: 全遷移ポイントの網羅確認'
  It 'generate-dashboard.sh の呼び出し総数が 9 である (既存4 + Orchestrator責務1 + Workflow 3 in_progress遷移1 + Workflow 6 skip説明1 + Workflow 6 post-merge1 + Continuation Flow1)'
    When call check_total_dashboard_invocations_is_nine
    The status should be success
  End
End
